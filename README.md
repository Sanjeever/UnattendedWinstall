...
这是为了纯内网实验环境下简单直接地通过 RDP 远程连接。

### SSH 配置

`autounattend.xml` 不会自动安装或配置 OpenSSH Server。如需使用 SSH，请在系统安装完成并联网后，以管理员身份打开 PowerShell，执行：
...
- Windows 可按需获取 OpenSSH Server 等 Features on Demand，并继续扫描、下载和安装系统更新。
...
本 fork 主要保留上游的 Winhance 精简和优化能力，同时增加实验机无人值守安装、空密码 Administrator 自动登录、RDP、SSH 手工配置说明、WSL2 / Hyper-V、保留 Edge / Store 等本地定制。
